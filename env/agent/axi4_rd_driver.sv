...
// =============================================================================
// axi4_rd_driver.sv
// Driver cho AXI4 Read path: AR channel + R channel
// =============================================================================
...
class axi4_rd_driver extends uvm_driver #(axi4_rd_seq_item);
        ...
        forever begin
            ...
            fork
                ...
                begin
                    ...
                    `uvm_info(get_type_name(), "Reset detected — deassert all RD signals", UVM_MEDIUM)
                end
            join_any
            
            // Ép dừng tiến trình giao dịch nếu reset về 0
            disable fork;
            
            // Clean up item_port nếu bị treo ngang chừng
            if (tr != null) begin
                seq_item_port.item_done();
                tr = null;
            end
    ...
    virtual task drive_ar_channel(axi4_rd_seq_item tr);
        ...
        `uvm_info(get_type_name(),
                  $sformatf("AR done: ARADDR=0x%0h ARID=0x%0h ARLEN=%0d", tr.araddr, tr.arid, tr.arlen),
                  UVM_HIGH)
    endtask

    // =========================================================================
    // Drive R channel (Logic gốc)
    // =========================================================================
    virtual task drive_r_channel(axi4_rd_seq_item tr);
        ...
        forever begin
            ...
            if (cfg.backpressure_pct > 0) begin
                ...
                if (bp_cycles > 0) begin
                    `uvm_info("RD_BP", $sformatf("R  beat[%0d] STALL %0d cycles | bp_pct=%0d%% | ARADDR=0x%0h", beat_cnt, bp_cycles, cfg.backpressure_pct, tr.araddr), UVM_LOW)
                    vif.master_cb.rready <= 1'b0;
            ...
            `uvm_info(get_type_name(),
                      $sformatf("R  beat[%0d]: RDATA=0x%0h RLAST=%0b RRESP=%0b", beat_cnt-1, vif.master_cb.rdata, vif.master_cb.rlast, vif.master_cb.rresp),
                      UVM_HIGH)
            ...
            if (vif.master_cb.rlast) begin
                if (beat_cnt != expected_beats)
                    `uvm_error(get_type_name(), $sformatf("** RTL BUG ** RLAST_EARLY: rlast tại beat[%0d] nhưng expected=%0d beats | ARADDR=0x%0h ARLEN=%0d", beat_cnt-1, expected_beats, tr.araddr, tr.arlen))
                break;
            end
            ...
            if (beat_cnt == expected_beats) begin
                `uvm_error(get_type_name(), $sformatf("** RTL BUG ** RLAST_MISSING: nhận đủ %0d beats nhưng rlast chưa assert | ARADDR=0x%0h ARLEN=%0d", expected_beats, tr.araddr, tr.arlen))
                break;
            end
        ...
        `uvm_info(get_type_name(),
                  $sformatf("R  done: RID=0x%0h BEATS=%0d/%0d RRESP=%0b", tr.rid, beat_cnt, expected_beats, tr.rresp),
                  UVM_MEDIUM)
    endtask
